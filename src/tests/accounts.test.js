...
describe('getAccounts', () => {
  ...
  it('returns parsed array from localStorage', () => {
    const accounts = [{ id: '1', token: 'gho_abc', login: 'alice', name: 'Alice', avatar_url: '', plan: null, lastQuota: null }];
...
describe('getSelectedAccount', () => {
  ...
  it('returns account matching selected id', () => {
    const a = { id: 'a1', token: 'gho_x', login: 'bob', name: '', avatar_url: '', plan: null, lastQuota: null };
    const b = { id: 'a2', token: 'gho_y', login: 'carol', name: '', avatar_url: '', plan: null, lastQuota: null };
...
describe('saveAccounts', () => {
  it('writes array to localStorage', () => {
    const accounts = [{ id: '1', token: 'gho_abc', login: 'alice', name: '', avatar_url: '', plan: null, lastQuota: null }];
...
describe('addAccountObject', () => {
  it('adds an account to the list', () => {
    const acct = { id: 'a1', token: 'gho_x', login: 'alice', name: 'Alice', avatar_url: '', plan: null, lastQuota: null };
    addAccountObject(acct);
    expect(getAccounts()).toEqual([acct]);
  });

  it('does not add duplicate tokens', () => {
    const a = { id: 'a1', token: 'gho_dup', login: 'alice', name: '', avatar_url: '', plan: null, lastQuota: null };
    addAccountObject(a);
  ...
  it('keeps existing selection when adding a second account', () => {
    const a = { id: 'a1', token: 'gho_x', login: 'alice', name: '', avatar_url: '', plan: null, lastQuota: null };
    const b = { id: 'a2', token: 'gho_y', login: 'bob', name: '', avatar_url: '', plan: null, lastQuota: null };
    addAccountObject(a);
    addAccountObject(b);
    expect(getSelectedId()).toBe('a1');
  });
...
describe('signOutAll', () => {
  it('removes all accounts and selection from localStorage', () => {
    saveAccounts([{ id: 'a1', token: 'gho_x', login: 'a', name: '', avatar_url: '', plan: null, lastQuota: null }]);
    saveSelectedId('a1');
    signOutAll();
    expect(getAccounts()).toEqual([]);
    expect(getSelectedId()).toBeNull();
...
describe('validateTokenFormat', () => {
  ...
  it('rejects partial prefix', () => expect(validateTokenFormat('gho_')).toBe(false));
});
...
describe('addAccounts', () => {
  ...
  it('adds a valid token and returns added count', async () => {
    ...
    const result = await addAccounts(['gho_abc123XYZ']);
  ...
  it('reports failure if API returns 401', async () => {
    ...
    const result = await addAccounts(['gho_badtoken123']);
    expect(result.added).toBe(0);
  ...
  it('adds multiple valid tokens', async () => {
    ...
    const result = await addAccounts(['gho_token1xxxxx', 'gho_token2xxxxx']);
...
describe('updateAccountQuota', () => {
  it('saves lastQuota on the matching account', () => {
    const a = { id: 'a1', token: 'gho_x', login: 'alice', name: '', avatar_url: '', plan: 'business', lastQuota: null };
    saveAccounts([a]);
  ...
  it('saves plan to account when provided', () => {
    const account = {
      id: 'acc-test-1', token: 'gho_test', login: 'testuser',
  ...
  it('does not overwrite existing plan when plan param is null', () => {
    const account = {
      id: 'acc-test-2', token: 'gho_test2', login: 'user2',
...
describe('getSelectedToken', () => {
  ...
  it('returns token of selected account', () => {
    const a = { id: 'a1', token: 'gho_selected', login: 'alice', name: '', avatar_url: '', plan: null, lastQuota: null };
    saveAccounts([a]);
    saveSelectedId('a1');
    expect(getSelectedToken()).toBe('gho_selected');
  });
});

describe('accountGhHeaders', () => {
  it('uses selected account token', () => {
    const a = { id: 'a1', token: 'gho_mytoken', login: 'alice', name: '', avatar_url: '', plan: null, lastQuota: null };
    saveAccounts([a]);
    saveSelectedId('a1');
    const h = accountGhHeaders();
    expect(h['Authorization']).toBe('Bearer gho_mytoken');
